...
public CalCFrame(String title) {
    ...
    super(title);
    setVisible(true); //bug #BP-2 
    addWindowListener(new CustomWindowAdapter()); //bug #P-1 
  ...
  for ( int i = 0; i < 19; i++ ) {
    ...
    getContentPane().add( buttons[i] ); //bug DC-1 & DC-3 to DC-5
...
public void processNumbers() {
  ...
  if ( firstpress ) {
    ...
    num1 = Double.parseDouble(input); // converts a string number to double;bug P-3
    ...
    // be calculated
      ...
      num2 = Double.parseDouble(input);			 //converts second num to double; bug P-4.3           //converts second num to double
    ...
    num3 = Double.parseDouble(input); //bugP-4.7
   ...
   double answer = 0.0;
        ...
        switch( oper ) {
          ...
          case SQRT:
            ...
            break;  
            default: //should never happen;  bug DC-2
            clearScreen();
...
public void showAnswer( String s )
{
    ...
    answer = Double.parseDouble(s); // bug P-4
    if ( decnumber )    
